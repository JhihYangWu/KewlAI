...
CKPT_PATH = "/workspace/jhihyangwu/MyAnimateAnyone/training_runs/00015/model_step_100000.pth"
...
def main():
    ...
    model = AnimateAnyone(sd_model, config, device, phase=2).to(device)
    ...
    with torch.no_grad():
        ...
        # denoise 4 frames at a time (constrained by what was done during training)
        # move forward by 2 frames each time 4 frames are denoised
        # when frames are known, use inpainting method like figure 3 of https://arxiv.org/pdf/2211.10658
        FRAMES_PER_STEP = 4
        FORWARD_STEP_SIZE = 3
        gen_images = []
        known_latents = []
        ...
        for x in trange(0, time_frames - FRAMES_PER_STEP + FORWARD_STEP_SIZE, FORWARD_STEP_SIZE):
            # move back x for last frames to not out of bounds
            x = min(x, time_frames - FRAMES_PER_STEP)

            # get pose guidance
            pose_guidance_i = model.pose_guider(pose_sequence[x:x+FRAMES_PER_STEP])
            ...
            while t >= 0:
                v = model.denoising_unet(x_dec + pose_guidance_i, clip_features, reference_features.copy(), torch.Tensor([ddim_timesteps[t]] * FRAMES_PER_STEP).to(device),
                                         batch_size, FRAMES_PER_STEP, phase=2)
                v -= pose_guidance_i
                ...
                # inpainting method
                for i in range(x, x + FRAMES_PER_STEP):
                    if i < len(known_latents):
                        pred_x0[i - x] = known_latents[i]
                
                x_dec = torch.sqrt(alphas_cumprod_t) * pred_x0 + torch.sqrt(1.0 - alphas_cumprod_t) * e_t
            ...
            new_gen_image = model.vae.decoder(pred_latent)
            new_gen_image = torch.permute(new_gen_image, (0, 2, 3, 1))  # (FRAMES_PER_STEP, H, W, C)
            # append to gen_images
            for i in range(x, x + FRAMES_PER_STEP):
                if i >= len(gen_images):
                    gen_images.append(new_gen_image[i - x])
                if i >= len(known_latents):
                    known_latents.append(pred_x0[i - x])
            
        assert len(known_latents) == time_frames, "known_latents should be the same length as time_frames"
        assert len(gen_images) == time_frames, "gen_images should be the same length as time_frames"
